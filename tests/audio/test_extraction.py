...
class TestVoiceExtractor:
    ...
    def test_extract_features(self):
        audio_features = self.extractor.extract_features(self.filepath, time=None)

        for feature in self.extractor.features:
            assert feature in audio_features and feature in self.reference_features
            assert pytest.approx(audio_features[feature], nan_ok=True) == np.array(self.reference_features[feature])
